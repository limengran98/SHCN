...
def  generate_HG(data, graph,variable_weight=False):
    n, _ = data.shape
    idx = np.array([i for i in range(n)], dtype=np.int32)
    idx_map = {j: i for i, j in enumerate(idx)}
    edges = np.array(list(map(idx_map.get, graph.flatten())),
                    dtype=np.int32).reshape(graph.shape)
    #edges = np.array(graph)
    H = np.zeros(shape=[n,n])
    H = H + np.eye(n,n)
    for i in range(edges.shape[0]):
        H[edges[i][1],edges[i][0]] = 1
        H[edges[i][0],edges[i][0]] = 1
        
    idx_ = np.argwhere(np.all(H[..., :] == 0, axis=0))
    H = np.delete(H, idx_, axis=1)
